...
> 根拠は差別化・蓄積・**用途固有性** — 深さの用途は「コンサルと対等に渡り合う知識武装」であり、
> カバレッジ台帳の3射影〔対峙資料・発注仕様書・納品物QC〕は Marlin 契約では手に入らない。
> ※旧根拠「私有情報の取り込みは内製だけ」は補正2で撤回 — VDR はコンサルが扱う）
> ②**Stage 0 は入力ゼロ**（P23 再改訂: Biz DD の要求項目は固定スペック。毎回変わるのは **DR の結果の側**であり、
> 薄さはリサーチ結果を固定スペックに突き合わせる**カバレッジ監査**で機械が検出する。人間の入力は任意の補強）
