...
## 2.5 ⚠️ 逆風の証拠 — 人間協働は自動的には勝たない（v0.3 追加）

「人間が関われば良くなる」は**研究に否定されている**。

- メタ分析: **人間+AI のチーム性能は、多くの場合 AI 単独に劣る** `[S-114]`
- **意思決定タスクでは損失、創造タスクでは利得**という傾向（Vaccaro et al.）`[S-114]`
- 原因は **過信（over-reliance）と不適切な依存** — 人間が「どこで自分の判断を足すべきか」を誤る `[S-114]`
- 補完性が成立する条件は、**人間が AI の正誤を識別し、それに応じて依存を変えられること** `[S-114]`
- 有効性が確認された介入 `[S-114]`:
  1. **AI の答えを見る前に、人間が先に判断する**
  2. **意図的に減速させる**

> **IP への反映**: 介入点は「置けばよい」のではなく、
> **上記の形式に合わせて設計しないと、むしろ精度を落とす**。
> 特に I1（仮説承認）は「AI が出した仮説を承認する」形にすると過信を招く。
> **人間が先に「この投資の死因を3つ書く」→ その後に AI の仮説集合と突き合わせる**、という順序にする。
...
| # | 介入点 | 人間がすること | 効果 |
|---|---|---|---|
| **I1'** | **人間が先に死因を書く**（AI の出力を見る前） | 「この投資が失敗するとしたら理由は」を3つ | **過信の回避**（AI 提示後の承認は過信を招く）`[S-114]`。LLM の主流バイアスも補正 `[S-061]` |
| **I1** | **仮説集合の突き合わせ** | 人間の3つ vs AI の集合。差分だけを議論する | 差分に注意を集中させる |
...
- `[S-114]` *When combinations of humans and AI are useful: A systematic review and meta-analysis*（Vaccaro et al.）／ *Toward a science of human–AI teaming: A complementarity framework* ／ *Complementarity in Human-AI Collaboration* arXiv:2404.00029 ／ CHI 2026 レビュー
