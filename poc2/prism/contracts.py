...
class Query(BaseModel):
    """gap から機械生成する検索クエリ(R-1 の第1段)。純関数 research.build_queries が作る。"""
    item_key: str
    text: str


class SearchHit(BaseModel):
    """検索の生の戻り。証拠ではない — 取得・スナップショットを経て初めて Source になる。"""
    url: str
    title: str = ""
    snippet: str = ""
...
class SearchClient(Protocol):
    def search(self, query: str, k: int) -> list["SearchHit"]: ...  # 失敗は [](例外にしない)
